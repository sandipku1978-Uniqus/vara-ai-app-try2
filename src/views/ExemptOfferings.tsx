...
export default function ExemptOfferings() {
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '48px', color: 'var(--text-muted)' }}><Loader2 size={24} className="spinner" style={{ marginBottom: '8px' }} /><div>Searching Form D filings...</div></div>
      ) : results.length > 0 ? (
        <>
          <AIResultsSummary
        ...
        <div>
          ...
          {recentLoading ? (
            ...
            <div style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}>Search for exempt offerings above.</div>
          )}
        </div>
      )}
    </div>
