...
export default function ExhibitSearch() {
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {EXHIBIT_TYPES.map(et => (
          <button key={et.value} onClick={() => toggleType(et.value)}
            style={{
              ...
              borderColor: selectedTypes.includes(et.value) ? '#B31F7E' : 'var(--input-border)',
              background: selectedTypes.includes(et.value) ? 'rgba(179,31,126,0.15)' : 'transparent',
              color: selectedTypes.includes(et.value) ? '#D66CAE' : 'var(--text-secondary)',
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '48px', color: 'var(--text-muted)' }}><Loader2 size={24} className="spinner" style={{ marginBottom: '8px' }} /><div>Searching exhibits...</div></div>
      ) : results.length > 0 ? (
        <>
          <AIResultsSummary
        ...
        <div style={{ textAlign: 'center', padding: '48px', color: 'var(--text-muted)' }}>No exhibits found.</div>
      ) : (
        <div>
          ...
          {recentLoading ? (
            <div style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}><Loader2 size={20} className="spinner" style={{ marginBottom: '8px' }} /><div>Loading recent exhibits...</div></div>
