      ...
      <div className="board-header">
        ...
        <div className="board-company-controls">
          <div className="ticker-selector glass-card" style={{ padding: '3px 10px', display: 'flex', alignItems: 'center', borderRadius: '6px', border: '1px solid var(--border-color)' }}>
            ...
            {(isLoading || boardLoading) && <Loader2 size={16} className="spinner" style={{ marginLeft: '6px' }} />}
          </div>
        ...
        <section className="board-main glass-card" style={{ overflow: 'auto' }}>
          ...
          {boardError && !boardLoading && (
            <div role="alert" style={{ padding: '28px', textAlign: 'center', color: 'var(--status-error)' }}>
              {boardError}
              <br />
              <button type="button" className="primary-btn sm" style={{ marginTop: '8px' }} onClick={() => fetchData(currentTicker)}>Retry</button>
            </div>
          )}
                  ...
                  <tbody>
                    {boardData.directors.map((dir, i) => (
                      <tr key={`${dir.name}-${i}`} style={{ borderBottom: '1px solid var(--border-color)', fontSize: '0.875rem' }}>
                        ...
                        <td style={{ padding: '9px 12px', color: 'var(--text-secondary)' }}>{dir.role}</td>
            ...
            <div className="tab-pane fade-in">
              ...
              <div className="board-diversity-grid">
                <div style={{ background: 'var(--surface-panel)', border: '1px solid var(--border-color)', padding: '16px', borderRadius: '6px' }}>
                  <h3 style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '12px', display: 'flex', justifyContent: 'space-between' }}>
                    Gender Identity
              ...
              <div style={{ marginTop: '16px', padding: '12px', background: 'color-mix(in srgb, var(--accent-primary) 5%, var(--surface-panel))', border: '1px solid color-mix(in srgb, var(--accent-primary) 20%, var(--border-color))', borderRadius: '6px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
          ...
          {activeTab === 'compensation' && companyData && boardData && !boardLoading && (
            <div className="tab-pane fade-in">
                  ...
                  <tbody>
                    {boardData.compensation.map((d, i) => (
                      <tr key={`${d.name}-${i}`} style={{ borderBottom: '1px solid var(--border-color)', fontSize: '0.875rem' }}>
                        ...
                        <td style={{ padding: '9px 12px', color: 'var(--text-secondary)' }}>{d.title}</td>
                        <td style={{ padding: '9px 12px', color: 'var(--text-secondary)', textAlign: 'right' }}>{d.salary}</td>
                        <td style={{ padding: '9px 12px', color: 'var(--text-secondary)', textAlign: 'right' }}>{d.stockAwards}</td>
                        <td style={{ padding: '9px 12px', color: 'var(--text-primary)', fontWeight: 700, textAlign: 'right' }}>{d.total}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="board-comp-grid">
                <div style={{ background: 'var(--surface-subtle)', border: '1px solid var(--border-color)', padding: '14px', borderRadius: '6px', textAlign: 'center' }}>
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={metric.label} style={{ borderBottom: '1px solid var(--border-color)', fontSize: '0.85rem' }}>
                        <th scope="row" style={{ padding: '8px 12px', fontWeight: 500, color: 'var(--text-secondary)' }}>{metric.label}</th>
                        {compareTickers.map(t => {
