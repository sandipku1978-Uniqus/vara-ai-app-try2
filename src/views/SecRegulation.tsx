...
export default function SecRegulation() {
  ...
  const tabStyle = (active: boolean) => ({
    ...
    background: active ? '#B31F7E' : 'var(--surface-panel)', color: active ? 'white' : 'var(--text-secondary)',
  });
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: '24px', fontSize: '0.9rem' }}>Search SEC rules, proposed regulations, and staff guidance documents.</p>
      ...
      {loading ? (
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
            ...
            <h2 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--text-primary)' }}>Recent {tab === 'rules' ? 'Rules & Releases' : 'Staff Guidance'}</h2>
          </div>
          {recentLoading ? (
            ...
            <div style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}>Search for SEC rules and regulations above.</div>
          )}
        </div>
      )}
    </div>
