import type { ExpectedSchemaIdentity } from '../../scripts/accuracy/schema-contract';

/** Complete bounded catalog evidence shared by release-artifact regressions. */
export function validSchemaContractEvidence(expected: ExpectedSchemaIdentity) {
  const relations = [
  ...
  const functions = [
    ...
    fn('urc_refresh_current_auditors', '', {
      anon: false, searchPath: 'search_path=pg_catalog', securityDefiner: true,
    }),
