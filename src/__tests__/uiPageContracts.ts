...
export const UI_PAGE_CONTRACTS = [
  ...
  {
    id: 'filing-ai-console',
    kind: 'product',
    access: 'authenticated',
    routePattern: '/filing-ai',
    representativePath: '/filing-ai',
    sourceFiles: ['src/app/filing-ai/page.tsx', 'src/views/filing-ai/FilingAIConsole.tsx'],
    intendedJob: 'Start a pre-flight integrity check against a draft or filed 10-K and reach the exception report for any check already run.',
    actions: [
      { id: 'filing-ai-console.run-check', interaction: 'Enter a registrant CIK, choose an input tier, and submit the pre-flight check.', expectedOutcome: 'The exception report for the completed run opens, or a stated reason is shown explaining why the run could not be completed.' },
      { id: 'filing-ai-console.reject-invalid-cik', interaction: 'Submit the form with a CIK that contains no digits.', expectedOutcome: 'The form reports that a valid CIK is required and no run is started.' },
      { id: 'filing-ai-console.choose-tier', interaction: 'Change the input tier selection.', expectedOutcome: 'The field explains what that tier can and cannot see before the check is run, not after.' },
      { id: 'filing-ai-console.open-run', interaction: 'Activate a run in the recent-checks table.', expectedOutcome: 'The exception report for that run opens with its findings and coverage statement.' },
      { id: 'filing-ai-console.open-catalog', interaction: 'Activate the rule catalog link.', expectedOutcome: 'The governed rule catalog opens.' },
    ],
    contentChecks: [
      { id: 'filing-ai-console.scope-language', assertion: 'The standing scope limitation is readable without opening anything.', expectedContent: 'Text stating the report evidences a management review control and is not an audit, review or attestation.' },
      { id: 'filing-ai-console.ipog', assertion: 'The input, process, output and governance layers are explained where the work starts.', expectedContent: 'Four labelled panels naming what each layer contributes to a run.' },
    ],
  },
  {
    id: 'filing-ai-run',
    // Reached from the console's recent-checks table, not from the nav, so it
    // is a dynamic route rather than a registered product destination.
    kind: 'dynamic',
    access: 'authenticated',
    routePattern: '/filing-ai/runs/[runId]',
    representativePath: '/filing-ai/runs/run_00000000-0000-0000-0000-000000000000',
    sourceFiles: ['src/app/filing-ai/runs/[runId]/page.tsx', 'src/views/filing-ai/FilingAIRunReport.tsx'],
    intendedJob: 'Read the exception report for one pre-flight check, verify each finding against the filed document, and dispose of it with a named rationale.',
    actions: [
      { id: 'filing-ai-run.filter-severity', interaction: 'Activate a severity filter chip.', expectedOutcome: 'The finding list narrows to that severity and the shown count updates.' },
      { id: 'filing-ai-run.filter-layer', interaction: 'Activate a check layer in the sidebar.', expectedOutcome: 'The finding list narrows to that layer.' },
      { id: 'filing-ai-run.expand-finding', interaction: 'Activate a finding heading.', expectedOutcome: 'The observed value, expected value, cited authority, next step and provenance pointer become readable.' },
      { id: 'filing-ai-run.open-provenance', interaction: 'Activate the provenance document link on a finding.', expectedOutcome: 'The filed document opens on EDGAR in a separate tab at the artefact the finding cites.' },
      { id: 'filing-ai-run.dispose-finding', interaction: 'Record a disposition with a name, role and rationale.', expectedOutcome: 'The finding shows the recorded disposition, or the request is refused with the governance requirement it did not meet.' },
      { id: 'filing-ai-run.missing-run', interaction: 'Open a run identifier that does not exist for this account.', expectedOutcome: 'A stated unavailability message and a route back to the console are shown rather than an empty report.' },
      { id: 'filing-ai-run.back-to-console', interaction: 'Activate the console route offered on an unavailable report.', expectedOutcome: 'The pre-flight console opens so the reader can start or find a run.' },
    ],
    contentChecks: [
      { id: 'filing-ai-run.coverage-statement', assertion: 'The coverage statement is readable before the findings.', expectedContent: 'Rules executed over rules applicable, the coverage percentage, and a list of what the run could not see.' },
      { id: 'filing-ai-run.gate-and-score', assertion: 'The readiness gate and the coverage-adjusted score are both visible.', expectedContent: 'A gate state and a readiness figure shown alongside the rule-coverage percentage that scaled it.' },
      { id: 'filing-ai-run.suppressed-rules', assertion: 'Rules that did not execute are listed with the gate that stopped each.', expectedContent: 'A table of rule identifiers, gate references and stated reasons.' },
    ],
  },
  {
    id: 'filing-ai-catalog',
    kind: 'product',
    access: 'authenticated',
    routePattern: '/filing-ai/catalog',
    representativePath: '/filing-ai/catalog',
    sourceFiles: ['src/app/filing-ai/catalog/page.tsx', 'src/views/filing-ai/FilingAICatalog.tsx'],
    intendedJob: 'Let a rule board member inspect every governed rule, its status on the promotion ladder, and whether a predicate for it is deployed.',
    actions: [
      { id: 'filing-ai-catalog.search-rules', interaction: 'Enter a rule identifier, name or authority in the search field.', expectedOutcome: 'The rule list narrows to matching rules and the shown count updates.' },
      { id: 'filing-ai-catalog.filter-layer', interaction: 'Choose a check layer.', expectedOutcome: 'Only rules in that layer remain listed.' },
      { id: 'filing-ai-catalog.filter-deployed', interaction: 'Restrict the list to deployed predicates.', expectedOutcome: 'Rules with no deployed predicate are removed from the list.' },
    ],
    contentChecks: [
      { id: 'filing-ai-catalog.unbound-reasons', assertion: 'Every rule without a deployed predicate states why.', expectedContent: 'A stated reason next to each rule marked as not deployed.' },
      { id: 'filing-ai-catalog.promotion-ladder', assertion: 'The rule board promotion ladder is visible.', expectedContent: 'The ordered statuses from draft through to live, with the requirement for live status stated.' },
    ],
  },
  {
    id: 'filing-ai-governance',
    kind: 'product',
    access: 'authenticated',
    routePattern: '/filing-ai/governance',
    representativePath: '/filing-ai/governance',
    sourceFiles: ['src/app/filing-ai/governance/page.tsx', 'src/views/filing-ai/FilingAIGovernance.tsx'],
    intendedJob: 'Create an engagement, name its team and roles, and work through the onboarding gate that must close before findings can be disposed of.',
    actions: [
      { id: 'filing-ai-governance.create-engagement', interaction: 'Enter a client name, registrant CIK, tier and four-eyes threshold, then create the engagement.', expectedOutcome: 'The new engagement appears in the list and opens with its onboarding gate outstanding.' },
      { id: 'filing-ai-governance.add-member', interaction: 'Add a team member with a name, email and role.', expectedOutcome: 'The member appears in the team table with their role and acknowledgement state.' },
      { id: 'filing-ai-governance.remove-member', interaction: 'Remove a team member.', expectedOutcome: 'The member is no longer listed and the team check re-evaluates.' },
      { id: 'filing-ai-governance.sign-off-step', interaction: 'Sign off an onboarding step.', expectedOutcome: 'The step is marked complete with the date, or is refused with the requirement it did not meet.' },
      { id: 'filing-ai-governance.select-engagement', interaction: 'Select a different engagement in the list.', expectedOutcome: 'That engagement’s onboarding gate and team replace the previous one.' },
    ],
    contentChecks: [
      { id: 'filing-ai-governance.roles', assertion: 'Each role states what it may do that the role below it may not.', expectedContent: 'A card per role listing its capabilities.' },
      { id: 'filing-ai-governance.team-shortfall', assertion: 'A team that could not dispose of a critical finding is told so.', expectedContent: 'A stated problem naming the four-eyes requirement when fewer than two members can dispose of a finding.' },
    ],
  },
  {
    id: 'support-center',
